...
int F_generic(const T** arg, T** res) {
	...
	OpenSim::Body* pelvis;
	pelvis = new OpenSim::Body("pelvis", 12.22455632690444282673, Vec3(-0.09131225898675764119, 0.00000000000000000000, 0.00000000000000000000), Inertia(0.10670666471985879031, 0.09041002429085311098, 0.06010034909805275771, 0., 0., 0.));
	model->addBody(pelvis);

	OpenSim::Body* femur_r;
	femur_r = new OpenSim::Body("femur_r", 9.65487715199702734026, Vec3(0.00000000000000000000, -0.20881942236445932837, 0.00000000000000000000), Inertia(0.20971183739426915715, 0.05497300591888609755, 0.22114496967939362149, 0., 0., 0.));
	model->addBody(femur_r);

	OpenSim::Body* tibia_r;
	tibia_r = new OpenSim::Body("tibia_r", 3.84839454716805873247, Vec3(0.00000000000000000000, -0.23814788838489062139, 0.00000000000000000000), Inertia(0.08512042857012629138, 0.00861337670054849493, 0.08630265674471138126, 0., 0., 0.));
	model->addBody(tibia_r);

	OpenSim::Body* talus_r;
	talus_r = new OpenSim::Body("talus_r", 0.10380025750959026576, Vec3(0.00000000000000000000, 0.00000000000000000000, 0.00000000000000000000), Inertia(0.00105044791266089752, 0.00105044791266089752, 0.00105044791266089752, 0., 0., 0.));
	model->addBody(talus_r);

	OpenSim::Body* calcn_r;
	calcn_r = new OpenSim::Body("calcn_r", 1.29750321886987807218, Vec3(0.10059769868939755222, 0.03017930960681926289, 0.00000000000000000000), Inertia(0.00147062707772525631, 0.00409674685937749979, 0.00430683644190968037, 0., 0., 0.));
	model->addBody(calcn_r);

	OpenSim::Body* toes_r;
	toes_r = new OpenSim::Body("toes_r", 0.22483135776577248266, Vec3(0.03480680374653154913, 0.00603586192136385240, -0.01760459727064457233), Inertia(0.00010504479126608974, 0.00021008958253217948, 0.00105044791266089752, 0., 0., 0.));
	model->addBody(toes_r);

	OpenSim::Body* femur_l;
	femur_l = new OpenSim::Body("femur_l", 9.65487715199702734026, Vec3(0.00000000000000000000, -0.20740666054336917568, 0.00000000000000000000), Inertia(0.20688383738234394915, 0.05423168552741054749, 0.21816279192223272942, 0., 0., 0.));
	model->addBody(femur_l);

	OpenSim::Body* tibia_l;
	tibia_l = new OpenSim::Body("tibia_l", 3.84839454716805873247, Vec3(0.00000000000000000000, -0.24024324981416839142, 0.00000000000000000000), Inertia(0.08662489465836918645, 0.00876561434043021670, 0.08782801819529098553, 0., 0., 0.));
	model->addBody(tibia_l);

	OpenSim::Body* talus_l;
	talus_l = new OpenSim::Body("talus_l", 0.10380025750959026576, Vec3(0.00000000000000000000, 0.00000000000000000000, 0.00000000000000000000), Inertia(0.00105044791266089752, 0.00105044791266089752, 0.00105044791266089752, 0., 0., 0.));
	model->addBody(talus_l);

	OpenSim::Body* calcn_l;
	calcn_l = new OpenSim::Body("calcn_l", 1.29750321886987807218, Vec3(0.10059769868939755222, 0.03017930960681926289, 0.00000000000000000000), Inertia(0.00147062707772525631, 0.00409674685937749979, 0.00430683644190968037, 0., 0., 0.));
	model->addBody(calcn_l);

	OpenSim::Body* toes_l;
	toes_l = new OpenSim::Body("toes_l", 0.22483135776577248266, Vec3(0.03480680374653154913, 0.00603586192136385240, 0.01760459727064457233), Inertia(0.00010504479126608974, 0.00021008958253217948, 0.00105044791266089752, 0., 0., 0.));
	model->addBody(toes_l);

	OpenSim::Body* torso;
	torso = new OpenSim::Body("torso", 27.84607988106773746040, Vec3(-0.02956147239050150102, 0.31532237216534936275, 0.00000000000000000000), Inertia(1.53053479697890826294, 0.78421094548495429155, 1.48579688599227499601, 0., 0., 0.));
	model->addBody(torso);

	OpenSim::Body* humerus_r;
	humerus_r = new OpenSim::Body("humerus_r", 2.10974023388242226673, Vec3(0.00000000000000000000, -0.18012275280759165730, 0.00000000000000000000), Inertia(0.01486673936709731990, 0.00512856461843362241, 0.01668743580892415729, 0., 0., 0.));
	model->addBody(humerus_r);

	OpenSim::Body* ulna_r;
	ulna_r = new OpenSim::Body("ulna_r", 0.63058656437076088253, Vec3(0.00000000000000000000, -0.15113582395465521846, 0.00000000000000000000), Inertia(0.00483463888598226980, 0.00100871263725085835, 0.00524432638104693847, 0., 0., 0.));
	model->addBody(ulna_r);

	OpenSim::Body* radius_r;
	radius_r = new OpenSim::Body("radius_r", 0.63058656437076088253, Vec3(0.00000000000000000000, -0.15113582395465521846, 0.00000000000000000000), Inertia(0.00483463888598226980, 0.00100871263725085835, 0.00524432638104693847, 0., 0., 0.));
	model->addBody(radius_r);

	OpenSim::Body* hand_r;
	hand_r = new OpenSim::Body("hand_r", 0.47488617810637545613, Vec3(0.00000000000000000000, -0.08538970281843806520, 0.00000000000000000000), Inertia(0.00145594121752065653, 0.00089282493944372091, 0.00218717626847273524, 0., 0., 0.));
	model->addBody(hand_r);

	OpenSim::Body* humerus_l;
	humerus_l = new OpenSim::Body("humerus_l", 2.10974023388242226673, Vec3(0.00000000000000000000, -0.18012275280759165730, 0.00000000000000000000), Inertia(0.01486673936709731990, 0.00512856461843362241, 0.01668743580892415729, 0., 0., 0.));
	model->addBody(humerus_l);

	OpenSim::Body* ulna_l;
	ulna_l = new OpenSim::Body("ulna_l", 0.63058656437076088253, Vec3(0.00000000000000000000, -0.15113582395465521846, 0.00000000000000000000), Inertia(0.00483463888598226980, 0.00100871263725085835, 0.00524432638104693847, 0., 0., 0.));
	model->addBody(ulna_l);

	OpenSim::Body* radius_l;
	radius_l = new OpenSim::Body("radius_l", 0.63058656437076088253, Vec3(0.00000000000000000000, -0.15113582395465521846, 0.00000000000000000000), Inertia(0.00483463888598226980, 0.00100871263725085835, 0.00524432638104693847, 0., 0., 0.));
	model->addBody(radius_l);

	OpenSim::Body* hand_l;
	hand_l = new OpenSim::Body("hand_l", 0.47488617810637545613, Vec3(0.00000000000000000000, -0.08538970281843806520, 0.00000000000000000000), Inertia(0.00145594121752065653, 0.00089282493944372091, 0.00218717626847273524, 0., 0., 0.));
	...
	st_hip_r[3].setFunction(new MultiplierFunction(new Constant(0.00000000000000000000), 1.29154538878016467152));
	st_hip_r[3].setAxis(Vec3(1.00000000000000000000, 0.00000000000000000000, 0.00000000000000000000));
	st_hip_r[4].setFunction(new MultiplierFunction(new Constant(0.00000000000000000000), 1.29154538878016467152));
	st_hip_r[4].setAxis(Vec3(0.00000000000000000000, 1.00000000000000000000, 0.00000000000000000000));
	st_hip_r[5].setFunction(new MultiplierFunction(new Constant(0.00000000000000000000), 1.32308129495307058576));
	st_hip_r[5].setAxis(Vec3(0.00000000000000000000, 0.00000000000000000000, 1.00000000000000000000));
	OpenSim::CustomJoint* hip_r;
	hip_r = new OpenSim::CustomJoint("hip_r", *pelvis, Vec3(-0.07268300829899254634, -0.10137339756535512780, 0.10222126084807423019), Vec3(0.00000000000000000000, 0.00000000000000000000, 0.00000000000000000000), *femur_r, Vec3(0.00000000000000000000, 0.00000000000000000000, 0.00000000000000000000), Vec3(0.00000000000000000000, 0.00000000000000000000, 0.00000000000000000000), st_hip_r);
	...
	st_walker_knee_r[3].setFunction(new MultiplierFunction(new PolynomialFunction(st_walker_knee_r_3_coeffs_vec), 1.22834954332034884672));
	...
	st_walker_knee_r[4].setFunction(new MultiplierFunction(new PolynomialFunction(st_walker_knee_r_4_coeffs_vec), 1.22834954332034884672));
	...
	st_walker_knee_r[5].setFunction(new MultiplierFunction(new PolynomialFunction(st_walker_knee_r_5_coeffs_vec), 1.22834954332034884672));
	...
	walker_knee_r = new OpenSim::CustomJoint("walker_knee_r", *femur_r, Vec3(-0.00552757294494156913, -0.50313197294401490467, -0.00214961170081061055), Vec3(-1.64156999999999997364, 1.44618000000000002103, 1.57079999999999997407), *tibia_r, Vec3(-0.01031872988219125799, -0.00450886532331500403, -0.00189388160578780125), Vec3(-1.64156999999999997364, 1.44618000000000002103, 1.57079999999999997407), st_walker_knee_r);

	OpenSim::PinJoint* ankle_r;
	ankle_r = new OpenSim::PinJoint("ankle_r", *tibia_r, Vec3(-0.01275564479833372379, -0.51022579193334893777, 0.00000000000000000000), Vec3(0.17589499999999999580, -0.10520799999999999597, 0.01866220000000000032), *talus_r, Vec3(0.00000000000000000000, 0.00000000000000000000, 0.00000000000000000000), Vec3(0.17589499999999999580, -0.10520799999999999597, 0.01866220000000000032));

	OpenSim::PinJoint* subtalar_r;
	subtalar_r = new OpenSim::PinJoint("subtalar_r", *talus_r, Vec3(-0.04906149765081917885, -0.04220073460020226652, 0.00796733773620028445), Vec3(-1.76818999999999992845, 0.90622300000000000075, 1.81960000000000010623), *calcn_r, Vec3(0.00000000000000000000, 0.00000000000000000000, 0.00000000000000000000), Vec3(-1.76818999999999992845, 0.90622300000000000075, 1.81960000000000010623));

	OpenSim::PinJoint* mtp_r;
	mtp_r = new OpenSim::PinJoint("mtp_r", *calcn_r, Vec3(0.17986868525664279073, -0.00201195397378795080, 0.00108645514584549351), Vec3(-3.14158999999999988262, 0.61990100000000003533, 0.00000000000000000000), *toes_r, Vec3(0.00000000000000000000, 0.00000000000000000000, 0.00000000000000000000), Vec3(-3.14158999999999988262, 0.61990100000000003533, 0.00000000000000000000));
	...
	st_hip_l[3].setFunction(new MultiplierFunction(new Constant(0.00000000000000000000), 1.29154538878016467152));
	st_hip_l[3].setAxis(Vec3(1.00000000000000000000, 0.00000000000000000000, 0.00000000000000000000));
	st_hip_l[4].setFunction(new MultiplierFunction(new Constant(0.00000000000000000000), 1.29154538878016467152));
	st_hip_l[4].setAxis(Vec3(0.00000000000000000000, 1.00000000000000000000, 0.00000000000000000000));
	st_hip_l[5].setFunction(new MultiplierFunction(new Constant(0.00000000000000000000), 1.32308129495307058576));
	st_hip_l[5].setAxis(Vec3(0.00000000000000000000, 0.00000000000000000000, 1.00000000000000000000));
	OpenSim::CustomJoint* hip_l;
	hip_l = new OpenSim::CustomJoint("hip_l", *pelvis, Vec3(-0.07268300829899254634, -0.10137339756535512780, -0.10222126084807423019), Vec3(0.00000000000000000000, 0.00000000000000000000, 0.00000000000000000000), *femur_l, Vec3(0.00000000000000000000, 0.00000000000000000000, 0.00000000000000000000), Vec3(0.00000000000000000000, 0.00000000000000000000, 0.00000000000000000000), st_hip_l);
	...
	st_walker_knee_l[3].setFunction(new MultiplierFunction(new PolynomialFunction(st_walker_knee_l_3_coeffs_vec), 1.22003917966687747132));
	...
	st_walker_knee_l[4].setFunction(new MultiplierFunction(new PolynomialFunction(st_walker_knee_l_4_coeffs_vec), 1.22003917966687747132));
	...
	st_walker_knee_l[5].setFunction(new MultiplierFunction(new PolynomialFunction(st_walker_knee_l_5_coeffs_vec), 1.22003917966687747132));
	...
	walker_knee_l = new OpenSim::CustomJoint("walker_knee_l", *femur_l, Vec3(-0.00549017630850094814, -0.49972804799155301492, 0.00213506856441703563), Vec3(1.64156999999999997364, -1.44618000000000002103, 1.57079999999999997407), *tibia_l, Vec3(-0.01040951997376383248, -0.00454853690114152311, 0.00191054506014509034), Vec3(1.64156999999999997364, -1.44618000000000002103, 1.57079999999999997407), st_walker_knee_l);

	OpenSim::PinJoint* ankle_l;
	ankle_l = new OpenSim::PinJoint("ankle_l", *tibia_l, Vec3(-0.01286787626214078162, -0.51471505048563126472, 0.00000000000000000000), Vec3(-0.17589499999999999580, 0.10520799999999999597, 0.01866220000000000032), *talus_l, Vec3(0.00000000000000000000, 0.00000000000000000000, 0.00000000000000000000), Vec3(-0.17589499999999999580, 0.10520799999999999597, 0.01866220000000000032));

	OpenSim::PinJoint* subtalar_l;
	subtalar_l = new OpenSim::PinJoint("subtalar_l", *talus_l, Vec3(-0.04906149765081917885, -0.04220073460020226652, -0.00796733773620028445), Vec3(1.76818999999999992845, -0.90622300000000000075, 1.81960000000000010623), *calcn_l, Vec3(0.00000000000000000000, 0.00000000000000000000, 0.00000000000000000000), Vec3(1.76818999999999992845, -0.90622300000000000075, 1.81960000000000010623));

	OpenSim::PinJoint* mtp_l;
	mtp_l = new OpenSim::PinJoint("mtp_l", *calcn_l, Vec3(0.17986868525664279073, -0.00201195397378795080, -0.00108645514584549351), Vec3(-3.14158999999999988262, -0.61990100000000003533, 0.00000000000000000000), *toes_l, Vec3(0.00000000000000000000, 0.00000000000000000000, 0.00000000000000000000), Vec3(-3.14158999999999988262, -0.61990100000000003533, 0.00000000000000000000));
	...
	st_back[3].setFunction(new MultiplierFunction(new Constant(0.00000000000000000000), 1.29154538878016467152));
	st_back[3].setAxis(Vec3(1.00000000000000000000, 0.00000000000000000000, 0.00000000000000000000));
	st_back[4].setFunction(new MultiplierFunction(new Constant(0.00000000000000000000), 1.29154538878016467152));
	st_back[4].setAxis(Vec3(0.00000000000000000000, 1.00000000000000000000, 0.00000000000000000000));
	st_back[5].setFunction(new MultiplierFunction(new Constant(0.00000000000000000000), 1.32308129495307058576));
	st_back[5].setAxis(Vec3(0.00000000000000000000, 0.00000000000000000000, 1.00000000000000000000));
	OpenSim::CustomJoint* back;
	back = new OpenSim::CustomJoint("back", *pelvis, Vec3(-0.13005862065016257634, 0.10526094918558343005, 0.00000000000000000000), Vec3(0.00000000000000000000, 0.00000000000000000000, 0.00000000000000000000), *torso, Vec3(0.00000000000000000000, 0.00000000000000000000, 0.00000000000000000000), Vec3(0.00000000000000000000, 0.00000000000000000000, 0.00000000000000000000), st_back);
	...
	st_acromial_r[3].setFunction(new MultiplierFunction(new Constant(0.00000000000000000000), 0.98538241301671669614));
	st_acromial_r[3].setAxis(Vec3(1.00000000000000000000, 0.00000000000000000000, 0.00000000000000000000));
	st_acromial_r[4].setFunction(new MultiplierFunction(new Constant(0.00000000000000000000), 0.98538241301671669614));
	st_acromial_r[4].setAxis(Vec3(0.00000000000000000000, 1.00000000000000000000, 0.00000000000000000000));
	st_acromial_r[5].setFunction(new MultiplierFunction(new Constant(0.00000000000000000000), 1.05067283651243648102));
	...
	acromial_r = new OpenSim::CustomJoint("acromial_r", *torso, Vec3(0.00310888151306774104, 0.36606956643571025856, 0.17861438220711420288), Vec3(0.00000000000000000000, 0.00000000000000000000, 0.00000000000000000000), *humerus_r, Vec3(0.00000000000000000000, 0.00000000000000000000, 0.00000000000000000000), Vec3(0.00000000000000000000, 0.00000000000000000000, 0.00000000000000000000), st_acromial_r);

	OpenSim::PinJoint* elbow_r;
	elbow_r = new OpenSim::PinJoint("elbow_r", *humerus_r, Vec3(0.01439212570608858557, -0.31345686261861671618, -0.01050612037050517242), Vec3(-0.02286269999999999969, 0.22801799999999999846, 0.00516889999999999971), *ulna_r, Vec3(0.00000000000000000000, 0.00000000000000000000, 0.00000000000000000000), Vec3(-0.02286269999999999969, 0.22801799999999999846, 0.00516889999999999971));

	OpenSim::PinJoint* radioulnar_r;
	radioulnar_r = new OpenSim::PinJoint("radioulnar_r", *ulna_r, Vec3(-0.00843551701093520762, -0.01631050539040199490, 0.03270753533465482060), Vec3(-1.56884000000000001229, 0.05642799999999999899, 1.53614000000000006096), *radius_r, Vec3(0.00000000000000000000, 0.00000000000000000000, 0.00000000000000000000), Vec3(-1.56884000000000001229, 0.05642799999999999899, 1.53614000000000006096));

	OpenSim::WeldJoint* radius_hand_r;
	radius_hand_r = new OpenSim::WeldJoint("radius_hand_r", *radius_r, Vec3(-0.01103125362646008648, -0.29573967108309351381, 0.01706665475231576579), Vec3(-1.57079999999999997407, 0.00000000000000000000, -1.57079999999999997407), *hand_r, Vec3(0.00000000000000000000, 0.00000000000000000000, 0.00000000000000000000), Vec3(-1.57079999999999997407, 0.00000000000000000000, -1.57079999999999997407));
	...
	st_acromial_l[3].setFunction(new MultiplierFunction(new Constant(0.00000000000000000000), 0.98538241301671669614));
	st_acromial_l[3].setAxis(Vec3(1.00000000000000000000, 0.00000000000000000000, 0.00000000000000000000));
	st_acromial_l[4].setFunction(new MultiplierFunction(new Constant(0.00000000000000000000), 0.98538241301671669614));
	st_acromial_l[4].setAxis(Vec3(0.00000000000000000000, 1.00000000000000000000, 0.00000000000000000000));
	st_acromial_l[5].setFunction(new MultiplierFunction(new Constant(0.00000000000000000000), 1.05067283651243648102));
	...
	acromial_l = new OpenSim::CustomJoint("acromial_l", *torso, Vec3(0.00310888151306774104, 0.36606956643571025856, -0.17861438220711420288), Vec3(0.00000000000000000000, 0.00000000000000000000, 0.00000000000000000000), *humerus_l, Vec3(0.00000000000000000000, 0.00000000000000000000, 0.00000000000000000000), Vec3(0.00000000000000000000, 0.00000000000000000000, 0.00000000000000000000), st_acromial_l);

	OpenSim::PinJoint* elbow_l;
	elbow_l = new OpenSim::PinJoint("elbow_l", *humerus_l, Vec3(0.01439212570608858557, -0.31345686261861671618, 0.01050612037050517242), Vec3(0.02286269999999999969, -0.22801799999999999846, 0.00516889999999999971), *ulna_l, Vec3(0.00000000000000000000, 0.00000000000000000000, 0.00000000000000000000), Vec3(0.02286269999999999969, -0.22801799999999999846, 0.00516889999999999971));

	OpenSim::PinJoint* radioulnar_l;
	radioulnar_l = new OpenSim::PinJoint("radioulnar_l", *ulna_l, Vec3(-0.00843551701093520762, -0.01631050539040199490, -0.03270753533465482060), Vec3(1.56884000000000001229, -0.05642799999999999899, 1.53614000000000006096), *radius_l, Vec3(0.00000000000000000000, 0.00000000000000000000, 0.00000000000000000000), Vec3(1.56884000000000001229, -0.05642799999999999899, 1.53614000000000006096));

	OpenSim::WeldJoint* radius_hand_l;
	radius_hand_l = new OpenSim::WeldJoint("radius_hand_l", *radius_l, Vec3(-0.01103125362646008648, -0.29573967108309351381, -0.01706665475231576579), Vec3(1.57079999999999997407, 0.00000000000000000000, 1.57079999999999997407), *hand_l, Vec3(0.00000000000000000000, 0.00000000000000000000, 0.00000000000000000000), Vec3(1.57079999999999997407, 0.00000000000000000000, 1.57079999999999997407));
	...
	Vec3 SmoothSphereHalfSpaceForce_s1_r_location(0.00209264783487049478, -0.01000000000000000021, -0.00421169983923776279);
	...
	Vec3 SmoothSphereHalfSpaceForce_s2_r_location(0.16333229405263557932, -0.01000000000000000021, -0.03160551840190639616);
	...
	Vec3 SmoothSphereHalfSpaceForce_s3_r_location(0.14639742129613919630, -0.01000000000000000021, 0.05683719490744468944);
	...
	Vec3 SmoothSphereHalfSpaceForce_s4_r_location(0.07290600742617847629, -0.01000000000000000021, 0.02901963283269722027);
	...
	Vec3 SmoothSphereHalfSpaceForce_s5_r_location(0.06604336817245036462, -0.01000000000000000021, -0.02064989931298610859);
	...
	Vec3 SmoothSphereHalfSpaceForce_s6_r_location(0.04953252612933776999, -0.01000000000000000021, 0.06808736281659177880);
	...
	Vec3 SmoothSphereHalfSpaceForce_s1_l_location(0.00209264783487049478, -0.01000000000000000021, 0.00421169983923776279);
	...
	Vec3 SmoothSphereHalfSpaceForce_s2_l_location(0.16333229405263557932, -0.01000000000000000021, 0.03160551840190639616);
	...
	Vec3 SmoothSphereHalfSpaceForce_s3_l_location(0.14639742129613919630, -0.01000000000000000021, -0.05683719490744468944);
	...
	Vec3 SmoothSphereHalfSpaceForce_s4_l_location(0.07290600742617847629, -0.01000000000000000021, -0.02901963283269722027);
	...
	Vec3 SmoothSphereHalfSpaceForce_s5_l_location(0.06604336817245036462, -0.01000000000000000021, 0.02064989931298610859);
	...
	Vec3 SmoothSphereHalfSpaceForce_s6_l_location(0.04953252612933776999, -0.01000000000000000021, -0.06808736281659177880);
